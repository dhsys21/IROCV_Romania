	...
	: TDataModule(Owner)
{
	...
	memset(pc_Interface_IR_Data1, 0, sizeof(unsigned char) * PC_D_INTERFACE_IR_LEN1 * 2);
	memset(pc_Interface_IR_Data2, 0, sizeof(unsigned char) * PC_D_INTERFACE_IR_LEN2 * 2);
	memset(pc_Interface_Ocv_Data1, 0, sizeof(unsigned char) * PC_D_INTERFACE_OCV_LEN1 * 2);
	memset(pc_Interface_Ocv_Data2, 0, sizeof(unsigned char) * PC_D_INTERFACE_OCV_LEN2 * 2);
...
//			ClientSocket_PLC->Active != ClientSocket_PLC->Active;
...
//			ClientSocket_PC->Active != ClientSocket_PC->Active;
...
//  ClientSocket_PLC->Active = false;
	ClientSocket_PLC->Close();
//  ClientSocket_PC->Active = false;
	ClientSocket_PC->Close();
}
//---------------------------------------------------------------------------
...
{
//  부득이하게 순간적으로 PLC와의 연결이 물리적으로 이상이 생겼을 경우
//  확실하게 연결을 끊는다.

//	TClientSocket *TempSoc;
//	TempSoc = static_cast<TClientSocket *>(Sender);
//	TempSoc->Active = false;
//	TempSoc->Close();
